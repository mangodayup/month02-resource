...
<img src="/home/tarena/.config/Typora/typora-user-images/image-20210218175438610.png" alt="image-20210218175438610" style="zoom:100%;" />
...
* 进程命令
  ...
  * 查看进程信息
    ...
    * %CPU,%MEM : 占有的CPU和内存
    * STAT ： 进程状态信息，S I 表示阻塞状态(首字母为大S)  ，R 表示就绪状态或者运行状态（处于运行和就绪的进程是快速切换的）<进程优先级 s进程会话组组长 l多线程
    * START : 进程启动时间
    * COMMAND : 通过什么程序启动的进程
...
```python
    ...
    参数 ： target 绑定要执行的目标函数 
        ...
        如果args和kwargs同时存在，那必须args 传相对靠前的参数
```
...
```
新进程和原有进行一模一样，代码段内存空间都一样

原进程会继续执行
新进程只会去运行所绑定的函数内容，函数执行完了，新进程就结束了。
新进程和原进程各自运行，共用一个终端，互不影响。
```
...
#放进去的必须是IO操作对象，而不能是行为和执行函数，比如放入sock而不能放入sendto等
功能: 监控IO事件，阻塞等待IO发生
