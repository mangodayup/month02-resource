...
filename = "students.txt"  # 定义保存学生信息的文件名  字典形式的字符串
# '{},{},{},{}...'
...
def main():
    ...
    while ctrl:
        ...
        option_str = re.sub("\D", "", option)  # 提取数字 "\D":匹配option的非数字字符用‘’替换掉
...
def save(student):
    ...
    for info in student:
        """
        {'id': '1002', 'name': '正超', 'english': 68, 'python': 98, 'c': 78}
        {'id': '1003', 'name': '小王', 'english': 36, 'python': 54, 'c': 89}
        """
        ...
        if os.path.exists(filename):  # 判断文件是否存在
            ...
            with open(filename) as file:  # 打开文件
                ...
                for list in student:
                    ...
                    # {'id': '1001', 'name': '张三三', 'english': 49, 'python': 69, 'c': 98}
...
"""
学生信息管理系统代码优化：
1、ID输入唯一，数据类型必须统一（数字类型）
2、修改学生信息需单一修改
3、数据存储用mysql，与Python中的pymysql进行交互，提高存储效率
4、重复代码重构
5、进阶操作：MVC架构-->面向对象编程
"""
