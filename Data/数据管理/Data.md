...
```python
read([size])  []中的参数是可选参数，一般都会写2的n次方，比如1024等
功能： 来直接读取文件中字符。
...
#### 1.2.5 with操作(上下文调度器或上下文管理器)

python中的with语句也可以用于访问文件，在语句块结束后会自动释放资源。

* with语句格式（含有\_\_enter__和\_\_exit\_\_）
...
* 文件偏移量控制
  ...
  参数：offset  代表相对于某个位置移动的字节数。负数表示向前移动，正数表示向后移动。
       whence是基准位置的默认值为 0，代表从文件开头算起，1代表从当前位置算起，2 代表从文件  末尾算起。(是字节)
   ```
	> 注意：必须以二进制方式打开文件时，基准位置才能是1或者2
	>
	> 如果输入的是中文，一个中文占3个字节，所以不能写偏移量为2等，因为偏移量的单位就是字节。
...
功能： 查看文件列表
参数： 指定目录
返回值：目录中的文件名列表 .当前文件夹 ..上一级文件夹 ../..上上级文件夹
```
...
```python
 ...
 #不超过三行核心代码将前后s消掉
"sfsjdjdjdksssss","sjsijiasjoss","sssshishiijs"
...
print(result.span())
print(result.group())
#<_sre.SRE_Match object; span=(0, 9), match='Alex:1996'>
#(0, 9)
#Alex:1996
```
...
```sql
e.g. 创建stu数据库，编码为utf8  8.0以后版本默认utf8
...
> 注意：
>
> 1. char：定长，即指定存储字节数后，无论实际存储了多少字节数据，最终都占指定的字节大小。默认只能存1字节数据。存取效率高。存取效率高。可以预先开辟空间，存储空间对齐，查找效率高。小字节长度建议用char。
...
* 字段约束
  * 如果你想设置数字为无符号则加上 UNSIGNED（不可以为负） 
...
约束： unsigned not null/default comment
```
...
练习 使用book表
1. 将呐喊的价格修改为45元
update book set price = 43 where name = "《呐喊》";
2. 增加一个字段出版时间 类型为 date 放在价格后面
alter table book add time date after price;
3. 修改所有老舍的作品出版时间为 2018-10-1
update book set time ="2018-10-1" where author = "老舍";
4. 修改所有中国文学出版社出版的但是不是老舍的作品出版时间为 2020-1-1
update book set time ="2020-1-1" where author != "老舍" and publisher = "中国文学出版社";
5. 修改所有出版时间为Null的图书 出版时间为 2019-10-1
update book set time ="2019-10-1" where time is  NULL;
6. 所有鲁迅的图书价格增加5元
update book set price = price + 5 where author = "鲁迅";
7. 删除所有价格超过70元或者不到40元的图书
delete from book where price < 40 or price > 70;
...
SELECT field1, field2,...fieldN 
	...
	```sql
e.g. 
	mysql> select * from class where name like 'A%';（不区分大小写）
	```
...
* as 用法
  ...
  在sql语句中as用于给字段或者表重命名
   ...
   where先于select执行，所以select name as n,score as s from class as c where c.s>17;不行，在筛选的时候，还不知道score是s，必须写c.score.
   ```
...
* 排序
  ...
  **默认情况ASC表示升序，DESC表示降序**
  ...
  select * from class where sex='m' order by age;（升序不需要加asc）
  ```

  复合排序：对多个字段排序，即当第一排序项相同时按照第二排序项排序

  ```sql
  select * from class order by score desc,age; （默认按照原有顺序，原先谁在前面就在前面）
  ```
...
* 限制
  ...
  FROM table_name
  WHERE field
  LIMIT [num] [OFFSET num] offset跳过几个。
  ```
...
*  联合查询
	...
	eg:
select * from class where sex = "m" union select * from class where score > 90
   等价于 or
   可以用 union all 来显示重复的，也可以用来查询不同字段，字段名称按照第一条的来写，只要查询的字段个数相同就可以。
...
* 子查询
	...
	* 子查询使用位置：
		...
		```
		>注意：  需要将子查询结果集重命名一下，方便where子句中的引用操作，必须进行as命名。
...
update sanguo set attack =300 where attack > 300 limit 2;
...
select * from sanguo where gender = "女" and attack between 180 and 250;
...
1. 统计每位作家出版图书的平均价格
select author,avg(price) as 平均价格 from book group by author;
2. 统计每个出版社出版图书数量
select publisher, count(name) as 数量 from book group by publisher;
3. 查看总共有多少个出版社
select count(distinct publisher) as 出版社数量 from book;
4. 筛选出那些出版过超过50元图书的出版社，并按照其出版图书的平均价格降序排序
select publisher , avg(price)  as 平均价格 from book
group by publisher having max(price) >50 order by 平均价格 desc;

如果写where price < 50 就会把所有小于50的图书给筛选掉了。
5. 统计同一时间出版图书的最高价格和最低价格
select time as 出版时间,max(price) as 最高价格,min(price) as 最低价格 from book group by time;
```
...
#### 3.9.2 索引分类(全文索引，空间索引)【索引方法：B树，B+树，红黑树，哈希索引】
...
* 主表和从表：若同一个数据库中，B表的外键与A表的主键相对应，则A表为主表，B表为从表。外：对外关联；键：自动建立一个索引。
...
* 级联动作
  ...
  * cascade ：数据级联更新  on delete cascade   on update cascade
    * 当主表删除记录或更改被参照字段的值时,从表会级联更新；当主表删除记录时，从表相关的记录都会被删除。
...
存储过程和函数是事先经过编译并存储在数据库中的一段sql语句集合，调用存储过程和函数可以简化应用开发工作，提高数据处理的效率。 有返回值的叫函数，没有返回值的叫存储过程。

#### 3.12.1 函数创建

```sql
delimiter 自定义符号　定义完之后 ;就不是结尾符号了　 --定义分隔符

create function 函数名(形参列表) returns 返回类型　　-- 注意是retruns 声明返回值类型
begin
...
函数体： 若干sql语句组成
return: 返回指定类型返回值
-- 函数不提供查询功能，如果提供写操作，要确认是否每次调用这个函数都要执行这个写操作。
-- 删除 drop function st;
```
...
2. 添加授权用户work,密码123,对所有库的所有表有所有权限
  mysql>grant all privileges on *.* to 'work'@'%' identified by '123' with grant option;（是否能给下一级分配权限）
...
如果选择的第一个，new environment using virtualenv，是一个完全没有第三方库的。
```

* pymysql使用流程

1. 建立数据库连接：db = pymysql.connect(...)
2. 创建游标对象：cur = db.cursor() 操作数据库并且得到结果的对象叫做游标。
